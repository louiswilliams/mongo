 ...
 * nature of this problem is that the stepping-down will abort the index build before stepdown
 * happens.
 *
...
const hangAfterAbortFailpointTimesEntered = res.count;
...
try {
    ...
    assert.commandWorked(primary.adminCommand({
        ...
        timesEntered: hangAfterAbortFailpointTimesEntered + 1,
        maxTimeMS: kDefaultWaitForFailPointTimeout
    }));
    ...
    assert.commandWorked(primary.adminCommand(
        {configureFailPoint: 'hangAfterIndexBuildAbortOnInterrupt', mode: 'off'}));
}
