...
Writers may conflict with each other when more than one operation writes to the same document
concurrently. To force the writer to retry, the storage engine may throw a WriteConflictException at
any point, up to and including the the call to commit(). Therefore, most WUOWs are enclosed in a
writeConflictRetry loop that retries the write transaction until it succeeds, accompanied by a
...
Collection writes (inserts, updates, and deletes) perform writes to both the collection's
RecordStore and index's SortedDataInterface in the same storage transaction, or WUOW. This ensures
that completed, not-building indexes are always consistent with collection data.
