...
* Scan all documents on the collection to be indexed
  ...
  * Periodically yields locks and storage engine snapshots
...
Both new and removed keys are recorded in the _side-writes_ table. For example, during an index
...
`commitQuorum` option can be provided to the `createIndexes` command and specifies the number of
nodes, including itself, a primary must wait to be ready before committing. The default value is
`"votingMembers"`, or all data-bearing nodes.
